...
preflight_rebase_conflict_message() {
  local label base_label task repo
  label=$1
  base_label=$2
  task=${label%%/*}
  repo=${label#*/}
  printf '%s\n' \
    ...
    "Resolve manually:" \
    "  git -C $label rebase \"\$(git -C $base_label rev-parse HEAD)\"" \
    "  git -C $label add <resolved-files>" \
    "  git -C $label rebase --continue" \
...
preflight_pull_diverged_message() {
  local label local_display remote_display
  label=$1
  local_display=$2
  remote_display=$3
  printf '%s\n' \
    ...
    "Inspect manually:" \
    "  git -C $label fetch origin" \
    "  git -C $label log --oneline --left-right $local_display...$remote_display"
}
