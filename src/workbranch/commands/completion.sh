...
cmd_complete_commands() {
  printf '%s\n' add completion config doctor finalize finder help ide init land list path pull push remove status sync terminal update version
}
...
_workbranch() {
  ...
  case "$cur" in
    -*)
      case "$cmd" in
        ...
        status|pull|push|land|finalize|sync|path|finder|ide|terminal) words='--repo' ;;
        *) words='' ;;
      esac
      COMPREPLY=( $(compgen -W "$words" -- "$cur") )
      return 0
  ...
  case "$cmd" in
    remove|update|push|land|finalize|path|finder|ide|terminal)
...
_workbranch() {
  ...
  if [[ "$cur" == -* ]]; then
    case "$cmd" in
      ...
      status|pull|push|land|finalize|sync|path|finder|ide|terminal) flags=(--repo) ;;
      *) flags=() ;;
    esac
