...
test_generated_workbranch_is_up_to_date() {
  ...
  if ! build_output=$("$APP_ROOT/scripts/build-workbranch.sh" "$generated" 2>&1); then
    rm -rf "$tmp_root"
...
test_run_test_output_uses_status_prefixes() {
  out=$(
    REPO_ROOT="$REPO_ROOT" APP_ROOT="$APP_ROOT" /bin/bash <<'SCRIPT'
set -u
. "$APP_ROOT/tests/lib/helpers.sh"
cleanup_fixture() { :; }
...
test_run_test_continues_after_fail_helper() {
  out=$(
    REPO_ROOT="$REPO_ROOT" APP_ROOT="$APP_ROOT" /bin/bash <<'SCRIPT' || true
set -u
. "$APP_ROOT/tests/lib/helpers.sh"
cleanup_fixture() { :; }
...
test_runner_fails_fast_when_no_case_files() {
  ...
  cp "$APP_ROOT/tests/run.sh" "$tmp_root/tests/run.sh"
  cp "$APP_ROOT/tests/lib/helpers.sh" "$tmp_root/tests/lib/helpers.sh"
