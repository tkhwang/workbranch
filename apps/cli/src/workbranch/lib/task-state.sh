...
task_plan_add() {
  ...
  TASK_PLAN_SUMMARIES[$index]=''
  TASK_PLAN_STATUS_SEEN[$index]=0
...
task_load_plans() {
  local task file line in_code current_plan leading mark item_text depth item_index title status_value summary_text
  task=$1
  task_plan_reset
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ "$current_plan" -ge 0 ] && [ "${TASK_PLAN_TOTAL[$current_plan]:-0}" -eq 0 ] && [[ $line =~ ^[[:space:]]*status:[[:space:]]*([^[:space:]]+) ]]; then
      ...
      case "$status_value" in
        todo|planning|in-progress|review|blocked|done)
          TASK_PLAN_STATUSES[$current_plan]=$status_value
          TASK_PLAN_STATUS_SEEN[$current_plan]=1
          ;;
        *) TASK_PLAN_STATUSES[$current_plan]='' ;;
      esac
      continue
    fi
    if [ "$current_plan" -ge 0 ] && [ "${TASK_PLAN_STATUS_SEEN[$current_plan]:-0}" -eq 1 ] && [ -z "${TASK_PLAN_SUMMARIES[$current_plan]:-}" ] && [ "${TASK_PLAN_TOTAL[$current_plan]:-0}" -eq 0 ] \
      && [[ ! $line =~ ^[[:space:]]*- ]] && [[ $line =~ [^[:space:]] ]]; then
      summary_text=${line%$'\r'}
      summary_text=$(printf '%s' "$summary_text" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')
      TASK_PLAN_SUMMARIES[$current_plan]=$summary_text
      continue
    fi
...
task_plans_json() {
  ...
  while [ $i -lt $TASK_PLAN_COUNT ]; do
    ...
    printf ',"summary":'
    json_string "${TASK_PLAN_SUMMARIES[$i]:-}"
    printf ',"items":'
...
- 계획을 포함한 의미 있는 작업을 시작하면 `status:`를 `todo`에서 `planning`으로 즉시 변경합니다.
- `blocked`는 `in-progress`에서만 사용하고, blocker가 해소되면 `in-progress`로 복원합니다.
- `status:` 바로 아래에 지금 하는 일을 나타내는 요약 한 줄을 유지하고, 작업 내용이 바뀌면 갱신합니다.
...
- When meaningful work starts, including planning, move `status:` from `todo` to `planning` immediately.
- Use `blocked` only from `in-progress`; when unblocked, restore `in-progress`.
- Keep a one-line summary of the current work directly below `status:`, and refresh it when the focus changes.
