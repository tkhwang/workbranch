...
- 2026-07-05 Post-review visual fix 3: compact+narrow overlap blocks now render time-only to avoid repeated task labels colliding in one-day mode. `cd apps/companion && pnpm test && pnpm typecheck && pnpm lint && pnpm build; cd ../.. && git diff --check` — PASS (13 files, 82 tests; build PASS; diff check PASS).
- 2026-07-05 Post-review visual fix 4: all one-day compact blocks now render time-only, even when wide, because close vertical stacking still made repeated task labels noisy. `cd apps/companion && pnpm test && pnpm typecheck && pnpm lint && pnpm build; cd ../.. && git diff --check` — PASS (13 files, 82 tests; build PASS; diff check PASS).
- 2026-07-05 Post-review visual fix 5: display lane assignment reserves a minimum visible duration, so any block that would overlap on screen is separated into columns regardless of task identity. Activity blocks render only task name and time; plan/activity text stays in selected detail. `cd apps/companion && pnpm exec biome check src/activity/calendar.ts src/activity/ActivityCalendarView.tsx tests/activity-calendar.test.tsx && pnpm test && pnpm typecheck && pnpm build && pnpm lint; cd ../.. && git diff --check` — PASS (13 files, 84 tests; typecheck/build PASS; full lint exit 0 with existing parseContract info diagnostics; diff check PASS).
- 2026-07-05 Post-review fix 6: `hourRange` now normalizes each session against its own containing day before computing visible hour bounds, preventing day 2/3 sessions in three-day mode from forcing the rail to 24 / 12 AM. Added a three-day `CalendarTimeline` regression test. `cd apps/companion && pnpm test && pnpm typecheck && pnpm lint && pnpm build; cd ../.. && git diff --check` — PASS (13 files, 85 tests; full lint exit 0 with existing parseContract info diagnostics; build PASS; diff check PASS).
- (실행 시 기록)
