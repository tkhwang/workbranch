...
Show configured repos, base branches, current branches, and task workspaces. `--json` emits a single machine-readable document with `schemaVersion`, `project`, `root`, and registered task workspaces only. Schema version 1 includes additive task progress fields for companion apps: `planTitle`, `status`, `progressDone`, `progressTotal`, `currentItem`, and `updatedAt`, alongside existing `memoTitle`, `notiCount`, and `repos`. `updatedAt` is the task brief mtime as epoch seconds. Stale or partial task-shaped directories are excluded from JSON and remain diagnostic concerns for `doctor`/status-style flows.
...
The generated task brief starts with a `#` memo title, may include a separated `status: todo|planning|in-progress|review|blocked|done` line and a separated `plan:` line naming the concrete Plan, and uses Markdown checklist items as Steps for progress. When `status:` is absent, progress may be derived from the checklist (`todo` for no completed checklist work or no checklist, `in-progress` for partial progress, `done` for all checklist items complete). The first unchecked Step is the current item. When `PREFERRED_LANGUAGE ko` is configured, generated task brief and AGENTS guidance text use Korean while preserving machine-readable `status:` values.
