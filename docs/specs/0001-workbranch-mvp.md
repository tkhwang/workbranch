...
The generated task brief starts with a `#` memo title, may include a separated `status: todo|planning|in-progress|review|blocked|done` line, and uses Markdown checklist items for progress. When `status:` is absent, progress may be derived from the checklist (`todo` for no completed checklist work or no checklist, `in-progress` for partial progress, `done` for all checklist items complete). The first unchecked checklist item is the current item. When `PREFERRED_LANGUAGE ko` is configured, generated task brief and AGENTS guidance text use Korean while preserving machine-readable `status:` values.
