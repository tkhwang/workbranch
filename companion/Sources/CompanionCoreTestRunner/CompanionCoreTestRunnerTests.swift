...
func runAppSourceInvariantTests() throws {
    ...
    try expect(rowView.contains("let candidates = row.plans.filter"), "RowView filters renderable plan candidates before choosing the current plan")
    try expect(rowView.contains("candidates.first(where: { $0.status != \"done\" })"), "RowView chooses the first non-done plan as current")
    try expect(rowView.contains("return [activePlan]"), "RowView renders only the active current plan")
    try expect(rowView.contains("return [candidates[candidates.count - 1]]"), "RowView falls back to the last plan when every plan is done")
